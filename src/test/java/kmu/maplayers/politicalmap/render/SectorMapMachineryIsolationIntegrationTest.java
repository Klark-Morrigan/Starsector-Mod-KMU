...
final class SectorMapMachineryIsolationIntegrationTest {
    ...
    private static SectorAPI installMachineryOnASectorHeldBy(
        ...
        var holder = SectorOwnershipFixtures.buildFaction(holderFactionId);
        var systems = new SectorOwnershipFixtures.SystemMarkets[systemIds.length];

        for (var index = 0; index < systemIds.length; index++) {

            systems[index] = listSystemMarkets(
                systemIds[index],
                SectorOwnershipFixtures.buildVisibleMarket(holder, HOLDING_COLONY_SIZE));
        }
        var sector = SectorOwnershipFixtures.buildSectorWithSystems(List.of(holder), systems);
        ...
        // The spotlight heal the install runs first is not what this suite is about, so it is handed a
        // registry with no view picked, under which it judges nothing.
        PoliticalMapInstaller.installAll(sector, mock(MapLayerViewRegistry.class));
    ...
    private static void handTheSharedSystemOf(SectorAPI sector, String factionId) {
        ...
        var faction = SectorOwnershipFixtures.buildFaction(factionId);
        var colony = SectorOwnershipFixtures.buildVisibleMarket(faction, HOLDING_COLONY_SIZE);
        var system = SectorOwnershipFixtures.findSystemIn(sector, SHARED_SYSTEM_ID);
    ...
    private static void driftTheSharedSystemPastTwoPollsOf(SectorAPI sector) {
        ...
        SectorOwnershipFixtures
            .findSystemIn(sector, SHARED_SYSTEM_ID)
            .getLocation()
            .x
    ...
    private static final class PoliticalMapCacheDriver {
        ...
        private OwnerMapClusters readTerritories() {
