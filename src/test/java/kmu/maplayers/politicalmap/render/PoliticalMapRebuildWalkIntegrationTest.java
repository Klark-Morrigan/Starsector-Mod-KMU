...
final class PoliticalMapRebuildWalkIntegrationTest {
    ...
    class RefreshDrawLists {
        ...
        void refreshDrawListsSamplesTheVisibilityRulesOnceForTheWholeRebuild() {
            ...
            seams
                ...
                .verify(MapVisibilityRules::readFromLunaSettings, times(1));
        }
    ...
    private SectorAPI buildContestedSectorWithAnEmptyNeighbour() {
        return buildContestedSectorStagedBy(SectorOwnershipFixtures::buildVisibleMarket);
    }
    ...
    private SectorAPI buildUndiscoveredSectorWithAnEmptyNeighbour() {
        return buildContestedSectorStagedBy(SectorOwnershipFixtures::buildUndiscoveredOpenMarket);
    }
    ...
    private SectorAPI buildContestedSectorStagedBy(ColonyStaging stageColony) {
        ...
        var hegemony = SectorOwnershipFixtures.buildFaction(HEGEMONY_ID);
        var tritachyon = SectorOwnershipFixtures.buildFaction(TRITACHYON_ID);

        return installAsTheRebuildsSector(SectorOwnershipFixtures.buildSectorWithSystems(
    ...
    private SectorAPI buildContestedSectorWithACoLocatedTwin() {
        ...
        var sector = installAsTheRebuildsSector(SectorOwnershipFixtures.buildSectorWithSystems(
            List.of(hegemony, tritachyon),
            listSystemMarkets(
                ALPHA_ID,
                SectorOwnershipFixtures.buildVisibleMarket(hegemony, HOLDING_COLONY_SIZE),
                SectorOwnershipFixtures.buildVisibleMarket(tritachyon, RIVAL_COLONY_SIZE)),
            listSystemMarkets(
                ALPHA_ID,
                SectorOwnershipFixtures.buildVisibleMarket(hegemony, HOLDING_COLONY_SIZE)),
            listSystemMarkets(
                BETA_ID,
                SectorOwnershipFixtures.buildVisibleMarket(tritachyon, HOLDING_COLONY_SIZE))));
    ...
    private void stageADifferentSectorAsTheRunningOne() {
        ...
        var tritachyon = SectorOwnershipFixtures.buildFaction(TRITACHYON_ID);
        var runningSector = SectorOwnershipFixtures.buildSectorWithSystems(
            List.of(tritachyon),
            listSystemMarkets(
                GAMMA_ID,
                SectorOwnershipFixtures.buildVisibleMarket(tritachyon, HOLDING_COLONY_SIZE)));

        SectorOwnershipFixtures.placeEverySystemInHyperspace(runningSector);
        seams
            .resolveGlobalSeam()
            .when(Global::getSector)
    ...
    private static void settleTheEmptyNeighbour(SectorAPI sector) {
        ...
        var colony = SectorOwnershipFixtures.buildVisibleMarket(
            SectorOwnershipFixtures.buildFaction(TRITACHYON_ID),
            HOLDING_COLONY_SIZE);
