    ...
    class MigrateLegacyKeys {
        ...
        void migrateLegacyKeysCarriesTheStoredMuteChoiceToTheSharedKeyAndDropsTheLegacyKey() {
            try (var memoryAccessMock = mockStatic(SectorMemoryAccess.class)) {
                ...
                when(memoryMock.contains(LEGACY_MUTE_KEY))
                    .thenReturn(true);
                when(memoryMock.getBoolean(LEGACY_MUTE_KEY))
                    .thenReturn(true);

                RecedePreferences.migrateLegacyKeys();

                verify(memoryMock)
                    .set(SHARED_MUTE_KEY, true);
                verify(memoryMock)
                    .unset(LEGACY_MUTE_KEY);
            }
        }
        ...
        void migrateLegacyKeysCarriesTheStoredDesaturateChoiceToTheSharedKeyAndDropsTheLegacyKey() {
            try (var memoryAccessMock = mockStatic(SectorMemoryAccess.class)) {
                ...
                when(memoryMock.contains(LEGACY_DESATURATE_KEY))
                    .thenReturn(true);
                when(memoryMock.getBoolean(LEGACY_DESATURATE_KEY))
                    .thenReturn(true);

                RecedePreferences.migrateLegacyKeys();

                verify(memoryMock)
                    .set(SHARED_DESATURATE_KEY, true);
                verify(memoryMock)
                    .unset(LEGACY_DESATURATE_KEY);
            }
        }
        ...
        void migrateLegacyKeysIsANoOpBeforeTheSectorExists() {
            ...
            try (var memoryAccessMock = mockStatic(SectorMemoryAccess.class)) {
                ...
                memoryAccessMock
                    .verify(SectorMemoryAccess::readSectorMemory);
            }
        }
    }
    ...
    class MigrateSharedKeysIntoFilterSet {
        ...
        void migrateSharedKeysCarriesTheStoredDesaturateChoiceIntoTheFilterKeyAndDropsTheSharedKey() {
            try (var memoryAccessMock = mockStatic(SectorMemoryAccess.class)) {
                ...
                when(memoryMock.contains(SHARED_DESATURATE_KEY))
                    .thenReturn(true);
                when(memoryMock.getBoolean(SHARED_DESATURATE_KEY))
                    .thenReturn(true);
        ...
        void migrateSharedKeysDoesNotOverwriteAFilterChoice() {
            ...
            try (var memoryAccessMock = mockStatic(SectorMemoryAccess.class)) {
                ...
                when(memoryMock.contains(FILTER_MUTE_KEY))
                    .thenReturn(true);
                when(memoryMock.contains(SHARED_MUTE_KEY))
                    .thenReturn(true);
        ...
        void migrateSharedKeysLeavesTheAllianceSetUntouched() {
            ...
            try (var memoryAccessMock = mockStatic(SectorMemoryAccess.class)) {
                ...
                verify(memoryMock, never())
                    .set(eq(ALLIANCE_MUTE_KEY), anyBoolean());
                verify(memoryMock, never())
                    .set(eq(ALLIANCE_DESATURATE_KEY), anyBoolean());
            }
        }
