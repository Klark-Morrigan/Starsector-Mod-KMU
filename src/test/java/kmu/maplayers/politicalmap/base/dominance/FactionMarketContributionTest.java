...
class FactionMarketContributionTest {
    ...
    class AddMarket {
        ...
        void foldsAPlanetMarketsWeightIntoTheFootprintAndItsSizeIntoTheTotal() {
            ...
            // since it sits on a planet) while its raw colony size lands in the market-size total.
            assertThat(FactionMarketContribution.EMPTY.addMarket(5, true, 3))
                    .isEqualTo(new FactionMarketContribution(new MarketFootprint(5, 5, 5), 3));
        }
        ...
        void keepsAStationMarketOutOfPlanetWeightWhileStillCountingItsSize() {
            ...
            assertThat(FactionMarketContribution.EMPTY.addMarket(4, false, 2))
                    .isEqualTo(new FactionMarketContribution(new MarketFootprint(4, 4, 0), 2));
        }

        @Test
        void accumulatesSuccessiveMarketsIntoBothTheFootprintAndTheSizeTotal() {
            // Two markets fold in one after another: the footprint sums and takes the heavier single
            // market, and the raw sizes sum.
            assertThat(FactionMarketContribution.EMPTY.addMarket(5, true, 3).addMarket(4, false, 2))
                    .isEqualTo(new FactionMarketContribution(new MarketFootprint(9, 5, 5), 5));
        }
    }
    ...
    class Merge {
        ...
        void mergesTheFootprintsAndSumsTheMarketSizes() {
            var left = new FactionMarketContribution(new MarketFootprint(5, 3, 2), 6);
            var right = new FactionMarketContribution(new MarketFootprint(4, 6, 1), 4);

            // The footprints fold as MarketFootprint.merge defines (total and planet weight sum, the
            // heaviest single market takes the larger), and the raw market sizes sum.
            assertThat(left.merge(right))
                    .isEqualTo(new FactionMarketContribution(new MarketFootprint(9, 6, 3), 10));
        }

        @Test
        void foldingEmptyLeavesTheContributionUnchanged() {
            var contribution = new FactionMarketContribution(new MarketFootprint(7, 5, 4), 8);
            ...
            assertThat(FactionMarketContribution.EMPTY.merge(contribution)).isEqualTo(contribution);
            assertThat(contribution.merge(FactionMarketContribution.EMPTY)).isEqualTo(contribution);
        }
    }
}
