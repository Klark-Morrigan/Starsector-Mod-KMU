 ...
 * and which pairings it refuses is {@code OwnerMapBandLayoutTest}'s.
 *
...
final class OwnerMapOverlayRendererTest {
    ...
    class RenderOnMap {
        ...
        void renderOnMapEmitsTheClustersForTheBandBeneathTheNebulae() {
            try (var clusterRendererMock = mockStatic(ClusterRenderer.class);
                ...
                // Both halves, since the framework hands the clusters out as two entries and
        ...
        void renderOnMapEmitsThePickerPreviewForTheBandBeneathTheNebulae() {
            ...
            try (var clusterRendererMock = mockStatic(ClusterRenderer.class);
                ...
                assertThat(previewHighlight.readPaintedFrames())
                    .containsExactly(PAINTING_FRAME);
            }
        }
        ...
        void renderOnMapEmitsTheClustersAboveTheNebulaeWhereTheFillsWereRaised() {
            ...
            try (var clusterRendererMock = mockStatic(ClusterRenderer.class);
                ...
                bandLayout = BandLayoutFixtures.buildLayout(
                    MapOverlayBand.ABOVE_STARSCAPE_NEBULAE,
                    MapOverlayBand.BENEATH_STARSCAPE_NEBULAE,
                    MapOverlayBand.ABOVE_STARSCAPE_NEBULAE,
                    MapOverlayBand.ABOVE_STARSCAPE_NEBULAE);
    ...
    private static OwnerMapCache buildCacheMock() {

        var clustersMock = mock(OwnerMapClusters.class);
        var paintedCellsMock = mock(PaintedCellStore.class);

        when(clustersMock.getStyledCellByCellKey())
            .thenReturn(Map.of());
        ...
        when(clustersMock.getPaintedCells())
        ...
        when(clustersMock.getGlobalStyle())
        ...
        when(cacheMock.isDebug())
            .thenReturn(false);
        when(cacheMock.getClusters())
            .thenReturn(clustersMock);
    ...
    // clusters, never both, so this is the other of the two states the lower band has to paint.
    // The loops are stubbed empty for the reason the styled cells are - the one-shot debug line
    // counts them.
    private static OwnerMapCache buildDebugCacheMock() {
