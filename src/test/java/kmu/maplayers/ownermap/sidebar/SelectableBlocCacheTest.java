...
/**
 * Pins what an owner map counts as a reason to rebuild its picker: the view's own content
 ...
 * same read. Pins its lifetime beside that - one memo per machinery, so two sectors' sidebars
...
final class SelectableBlocCacheTest {
    ...
    private static final BlocPickerRead<RankedBloc<BlocMetricsFake>> READ =
        new BlocPickerRead<>(
            new ListPicker<>(
    ...
    // The layers a memo is resolved for, two of them for the case about their keeping apart.
    ...
    private static final BlocPickerRead<RankedBloc<BlocMetricsFake>> OTHER_READ =
        new BlocPickerRead<>(
            new ListPicker<>(
                List.of(new RankedBloc<>(
    ...
    class ResolveBlocCacheIn {
        ...
        void resolveBlocCacheInAnswersOneCachePerMachinery() {
            ...
            assertThat(SelectableBlocCache.resolveBlocCacheIn(machinery, LAYER_ID))
                .isSameAs(SelectableBlocCache.resolveBlocCacheIn(machinery, LAYER_ID));
        }
        ...
        void resolveBlocCacheInAnswersEachLayerAMemoOfItsOwn() {
            ...
            assertThat(SelectableBlocCache.resolveBlocCacheIn(machinery, LAYER_ID))
                .isNotSameAs(SelectableBlocCache.resolveBlocCacheIn(machinery, OTHER_LAYER_ID));
        }
        ...
        void resolveBlocCacheInAnswersAFreshCacheAfterTheMachineryIsDisposed() {
            ...
            var machinery = new SectorMapMachinery(mock(SectorAPI.class));
            var cache = SelectableBlocCache.resolveBlocCacheIn(machinery, LAYER_ID);
    ...
    class ResolveBlocPickerRead {
        ...
        void resolveBlocPickerReadRecomputesWhenTheViewsContentRevisionMoves() {
            ...
            // A membership change bumps the grouped view's content revision between the two calls,
        ...
        void resolveBlocPickerReadKeysTheMemoOnItsOwnMachineryBoard() {
            ...
            new SelectableBlocCache(machinery).resolveBlocPickerRead(viewMock);

            verify(viewMock)
                .getContentRevision(machinery.resolveRefreshBoard());
        }
    ...
    // A cache over one sector's machinery, which is the only way a cache exists - the machinery
    ...
    private static OwnerPaintedView stubViewAnswering(SectorAPI sectorMock, String viewId) {
    ...
    private static OwnerPaintedView stubViewAnsweringPerSector(
