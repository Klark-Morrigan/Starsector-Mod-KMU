...
public final class SidebarLayout {
    ...
    private static BodyLayout computeBodyLayout(float contentX, float bodyTopY,
        ...
        for (var spec : specs) {
            ...
            contentWidth = Math.max(contentWidth, rowWidth + trailingWidth(spec, measurer));
        }
    ...
    private static float measureRowWidth(SidebarControlSpec spec, LineWidthMeasurer measurer) {
        return switch (spec.kind()) {
            ...
            case RADIO -> spec.labels().size() * radioSegmentWidth(spec, measurer);
    ...
    // padding.
    private static float radioSegmentWidth(SidebarControlSpec spec, LineWidthMeasurer measurer) {
        var widest = 0f;
    ...
    private static float trailingWidth(SidebarControlSpec spec, LineWidthMeasurer measurer) {
