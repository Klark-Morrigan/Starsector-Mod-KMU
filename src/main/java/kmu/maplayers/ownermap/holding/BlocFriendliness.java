 ...
 * only who happens to stand in one system is not the fact it purports to be: the same two groups
 ...
 * disposition as a predicate over a pair of them, so the rule holds no game state of its own and
...
public final class BlocFriendliness {
    ...
    public boolean areBlocsFriendly(Set<String> blocFactionIds, Set<String> otherBlocFactionIds) {
        ...
        // Friendliness is a positive statement about two blocs, so a bloc made of nobody leaves nothing
    ...
    // a positive statement: there is no pair to make it of. Which is why this is not simply the
