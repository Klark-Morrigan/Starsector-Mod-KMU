...
/**
 * What a box knows about one system's colonies beyond what the painting mechanic made of them: what
 * kind of place each is, whether the player has found it, whether a concealed one is concealed in
 * name only, and how old its news of each is.
 *
 * <p>For an account whose rows arrive as scores rather than as colonies. Such a row carries the ID
 ...
 * walk of the system the box already makes, and a line asks them together - so passed apart, one
 * could be folded off a second walk, and the box would state a kind and a date read from two
...
public final class SystemColonyReading {
     ...
     * leading finding is the painting mechanic's own outcome and a concealment travels on the
     * breakdown, so a reading folded off the system could state neither - which is also why the
     * kind and the discovery answer stay separately readable ({@link #readKindOf},
     * {@link #isDiscoveredColony}): an account that has the place itself in hand fills them in from
     * that instead.
     *
