 ...
 *
 * <p>One read for every box, because boxes name the same colonies of the same system: a word
 * resolved separately at each could have one box call a world collapsed while another lists it as
 * governed, which is the one disagreement a shared account cannot survive.
 *
 * <p>It applies the wording as well as resolving it, so the sharing reaches all the way to the line.
 ...
 *
 * <p>The order runs from what a place <em>is</em> to how it is concealed: the layer's own leading
 * finding, then its kind, then the three ways it may be out of sight. Two of those never join what
 * stands above them. {@code undiscovered} displaces {@code hidden}, an undiscovered colony being
 * concealed from the player by that fact alone; {@code unlisted} is a fallback and speaks only where
