 ...
 * is quietly stating they are equally current - which matters most for the colonies that reached
...
public final class ColonyObservationNotes {
    ...
    // always a box that lists a colony: one that hangs nothing beneath a faction would run an eager
    // fold on every hover of every system to answer nothing. A memo on a per-box value, like the
    // one the pass's own classification keeps.
    ...
    Optional<String> resolveLastSeenNote(String colonyId) {
        ...
        return ObservationNotes.resolveNoteForAxes(
            clock,
            List.of(new ObservationAxis(KmuStringKeys.OWNER_MAP_TOOLTIP_LAST_SEEN, recency)));
    }
