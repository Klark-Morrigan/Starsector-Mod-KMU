...
import kmu.maplayers.base.render.MapFrameCompositor;
...
/**
 * The owner map's {@link MapFrameCompositor}: composes the overlay's sub-layers over the
 * {@link OwnerMapCache}'s current draw lists, in one canonical order, bottom to top: the fills (or the
 * debug border-tracing overlay where it has replaced the base view), then the borders, then the
 * picker preview, then the hover highlight, then the cluster-anchor debug overlay, then the per-cell
 * presence bands, then the debug band-path overlay across them, then the faction names. That order
 * is fixed here and is not the player's - it is what makes each sub-layer legible against the ones
 * under it - and each layer still draws only under its own toggle, the debug facilities composing
 ...
 */
final class OwnerMapOverlayRenderer implements MapFrameCompositor {
    ...
    // The draw lists every band is emitted from. Held rather than handed per pass: the compositor and
    // the cache are made together for one sector's machinery, so there is only ever one to paint.
    ...
    /**
     * @param cache            the draw lists every band is emitted from, made for the same sector's
     *                         machinery as this compositor
     * @param hoverState       the hover holder of that sector, from its installed machinery
     ...
     */
    @Override
    public void renderBand(MapFrame mapFrame, MapOverlayBand band) {

        logFirstRenderOnce(mapFrame);
        ...
        if (cache.isDebug()) {
            ...
            renderProductionBaseView(mapFrame, isPaintingFills, layout.borderBand() == band);
        }
    ...
    // The production base view and the two highlights over it, in stack order. Apart from the band
    // dispatch because the debug overlay replaces all four at once: none has anything to draw from
    // once the cache built the overlay instead.
    private void renderProductionBaseView(
            MapFrame mapFrame,
            boolean isPaintingFills,
            boolean isPaintingBorders) {
        ...
        // After the fills wherever the two meet, which the layout guarantees: the borders can be
        // lifted clear of a fogged fill, but never sunk under their own fill.
        ...
        // switch: it answers a pointer on the sidebar rather than one on the map, so a player who has
        // switched the map's own cursor feedback off still sees what a row would spotlight. Its own
        // style tier is where it is turned down to nothing.
        previewHighlight.renderPreviewOnMap(
            cache.getClusters(),
            mapFrame);

        // Gated on the effects switches alone, though the hover it reads may have been published for
        // the box's sake: with only the tooltip on, the cursor is still resolved every frame and
        // nothing may be painted over the map for it.
        if (hoverGates.isHoverEffectsEnabled()) {
            renderHoverHighlight(mapFrame);
        }
    }
    ...
    private void renderHoverHighlight(MapFrame mapFrame) {
    ...
    private void renderPresenceBands(MapFrame mapFrame) {
    ...
    // LunaLib to see it.
    private void logFirstRenderOnce(MapFrame mapFrame) {
        ...
        LOG.debug("Owner map render renderBand fired: "
