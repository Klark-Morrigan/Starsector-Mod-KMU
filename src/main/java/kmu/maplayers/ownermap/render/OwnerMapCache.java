 ...
 * override flips (each changes which systems seed a cell). The clusters are rebuilt in full when
 ...
 *
 * <p>Which of those a frame owes is {@link OwnerMapRebuildDecider}'s answer, asked first and
...
public final class OwnerMapCache {
    ...
    private static final ProfileSection REBUILD_DRAWABLES_SECTION = ProfileSection.registerSection(
        "ownerMap.rebuildDrawables", RebuildStepTerms.LOGGED_EVERY_CALL);
    ...
    private final OwnerMapRebuildDecider decider;
    ...
    private void rebuildStaleHalves(OwnerPaintedView view, ScreenMemoryScope memoryScope) {
    ...
    // rebuilds the clusters under the newly-selected view.
    //
    ...
    private void rebuildWhatIsStale(StaleHalves staleHalves, OwnerPaintedView view) {
    ...
    // clusters; that batch opens a reading of its own, since none was opened for this frame.
    ...
    // holding, which would overwrite the spotlit keys and corrupt the spotlight, so a filtered
    ...
    private void rebuildGeometry(
        ...
        // recorded below.
        LOG.debug("Owner map geometry stale; rebuilding cut " + cellGeometry.revision()
