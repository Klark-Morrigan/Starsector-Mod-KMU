...
final class ClusterLabelStyling {
    ...
    // cluster border resolves to, so the name inherits the border's own colour rather
    ...
    // The per-bloc name estimators one rebuild fits against: each bloc's display name (a
    // faction's, or a group's) as the active view resolves it, measured with the label font, or
    // the aspect stand-in when the font or the name will not resolve. Cached per bloc ID because every cluster of a
    ...
    // for the bloc - a faction ID is not always what the label reads (a group's bloc ID
