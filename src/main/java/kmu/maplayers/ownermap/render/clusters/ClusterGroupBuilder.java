 ...
 *
 * <p>What it produces is a framework draw record, but what it decides is the owner map's throughout -
 ...
 * bloc's cluster group includes a cell no star of its own sits in, and it is the cells that carry the
 * edges a border is traced from.
 */
public final class ClusterGroupBuilder {

    // Builds only; never instantiated.
    private ClusterGroupBuilder() {
    }

    /**
     * Builds one bloc's cluster group from its member cells.
     *
     * @param clusters    the build the bloc belongs to, read for who holds its members and for
     ...
     * @param blocId         the bloc's holder - a faction or group ID from the view's grouping,
     *                       or one of the filter's synthetic spotlight keys
     ...
     */
    public static StyledClusterGroup buildClusterGroup(
        ...
        // One trace for the whole cluster group: the cluster border's rings, and - for a spotlit
        ...
        // The spotlighted bloc's whole footprint - won and contested systems alike - shares
        ...
        var tracedFill = new SplitFillBuilder(
            ...
            .traceFill(
                SpotlitBlocs.isSpotlitBloc(blocId),
    ...
    /**
     * Builds every bloc's cluster group into the clusters, keyed by its holder. Each bloc
     ...
     *
     * @param clusters   the pass state to write the clusters into
     ...
     */
    public static void buildAllClusterGroups(
            OwnerMapClusters clusters,
            CellGeometryCache geometryCache,
            CellGrouping cellGrouping) {
        ...
        for (var bloc : grouped.entrySet()) {
            ...
            var clusterGroup = buildClusterGroup(
                clusters,
                geometryCache,
                cellGrouping,
                bloc.getKey(),
