 ...
 *
 * <p>Where {@link OwnerStyleDecision} says only <em>whether</em> a bloc recedes to the
 * independent style, this is that decision mapped onto the pass's actual theme. Resolved once
 * per bloc and read by everything that draws it - its fill, its cluster border, and its
...
public record OwnerStyling(
     ...
     */
    public static OwnerStyling resolveFrom(RenderStyle renderStyle, OwnerStyleDecision decision) {

        var factionStyle = renderStyle.categoryStyle(OwnerMapCategory.FACTION);
        if (!decision.usesIndependentStyle()) {
            return new OwnerStyling(factionStyle, decision.adjustment());
        }
        return new OwnerStyling(
            applyDesaturatedFillOpacity(
                renderStyle.categoryStyle(OwnerMapCategory.INDEPENDENT),
                factionStyle,
                decision.adjustment()),
            decision.adjustment());
    }
    ...
    // cells without breaking the fill's uniformity.
