...
/**
 * Ranks an owner-map picker's blocs by where they stand with the player, and draws that standing
 * on each row.
 *
 * <p>A type of its own rather than another {@link BlocMetricSortMode} declaration, because the
 * standing is not a number any vocabulary's fold computes: it is a bloc-level fact read off the
 * sector, identical under every view, and a range rather than a single value. Declared as a metric it
 * would have to be summed onto every metrics record, asking each aggregator to carry something it
 * does not aggregate and duplicating one fact into every payload.
 *
 * <p>Being outside every vocabulary is also why it cannot fall through any one's numeric chain -
...
public final class BlocStandingSortMode<S extends BlocMetrics>
    ...
    // across every view rather than one per vocabulary, so the choice survives a view switch the way
    // every other mode's does.
    ...
    // group holding one hostile is not ranked as though it were the ally inside it.
    ...
    public String resolveLabelText() {
        return KmuStringKeys.get(KmuStringKeys.OWNER_MAP_CTL_SORT_ATTITUDE);
    }
     ...
     * faction, or a group that agrees - draws the single number they share. The two cases with no
    ...
    private static List<TextSpan> resolveMeasuredRuns(
        ...
        return List.of(
            lowestRun,
            new TextSpan(
                KmuStringKeys.get(KmuStringKeys.OWNER_MAP_CTL_SORT_ATTITUDE_RANGE_SEPARATOR),
