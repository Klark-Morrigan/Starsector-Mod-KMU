 ...
 * Spotlighting a bloc through the filter re-fuses clusters, re-traces borders and re-fits cluster
 ...
 * living somewhere under one layer's walk, something else under another's - so a bloc's set is
 * the cluster group the layer that built it actually paints.
 *
 ...
 * the walk counts that bloc, so a bloc's set size is the count its own stats state beside it -
 * whatever that layer calls the number.
 *
 ...
 *
 * <p>Plain data with no Starsector types, so it is built and read with nothing loaded.
 *
 ...
 *                           order stored and read back, so a caller building one has to hand over an
 *                           ordered collection to get an order it can rely on
 */
public record BlocPresenceIndex(
