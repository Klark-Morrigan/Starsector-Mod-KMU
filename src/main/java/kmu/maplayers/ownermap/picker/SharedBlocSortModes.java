...
/**
 * The sort modes every owner-map vocabulary offers rather than declares - the ones whose number
 ...
 * everywhere.
 *
 * <p>A mode holder beside the vocabularies' own, and named the same way for that reason: a
 * vocabulary's own numbers are declared where that vocabulary lives, as {@link BlocMetricSortMode}
 * values over its own accessors, and only a number every layer carrying it measures identically
 * belongs here. Anything else would put one mechanic's vocabulary in front of another's.
 */
public final class SharedBlocSortModes {
    ...
    // the storing vocabulary's default. One spelling for every vocabulary carrying the mode, so the
    // choice survives a view switch the way the standing ranking's does.
    ...
    public static <S extends SizedBlocMetrics> ListSortMode<RankedBloc<S>> declareMarketSizeMode(
        ...
        return new BlocMetricSortMode<>(
            ...
            KmuStringKeys.OWNER_MAP_CTL_SORT_MARKET_SIZE,
