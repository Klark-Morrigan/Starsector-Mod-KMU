...
/**
 * The source an owner-painted view resolves its per-system holder from, so the render
 ...
 * pass, and that is the whole of what makes the seam implementable by more than one mechanic: a
 * provider weighing markets and one reading any other rule off the sector both need the same
 * reading of the sector - which sector, the grouping, the reveal, and the one walk of each system -
 * while the rule that decides a winner is the provider's own to name. Handing the pass over rather
 * than the sector is also what makes two providers behind one view share a walk instead of taking
 * one apiece.
 */
