...
Nothing here knows what a grouping key means, or that ownership is what a layer paints at
...
the same ground (a cut interior to one grouping's ground with no star beyond it).
...
ground a grouping covers without a star in it, or space under no key - so
[`CellGrouping`](CellGrouping.java) makes "which grouping is this cell" two lookups: the
...
makes an open frontier (grouped ground facing ungrouped
...
a parallel flag per edge saying which edges are cluster borders.
...
"everyone who shares a key": one key's main body and its far-flung outlier come
...
The cache holds *raw* cells. Everything grouping-dependent - the inset, the seams,
the clusters - is derived downstream, which is why a system changing its key costs a
