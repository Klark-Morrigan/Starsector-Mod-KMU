...
all. Whatever divides the map arrives as an opaque *owner* per system, so
...
painted. A seam edge stays on the true cell border, so two same-owner cells meet
...
[`SystemClusters`](SystemClusters.java) groups systems into contiguous same-owner
clusters - the connected components of the adjacency graph. That is stricter than
"everyone who shares an owner": one owner's main body and its far-flung outlier come
...
the clusters - is derived downstream, which is why a system changing its owner costs a
