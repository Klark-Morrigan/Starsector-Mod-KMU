...
This file owns what the panel *is*: where it lands, the opener past its last tab, and the state its
lists and its fold are kept in. Two leaves own the rest.
...
than about a save or the process - and it is held by that sector's `SectorMapMachinery`, resolved
through `FilterHoverSlot.resolveHoverSlotIn`. A load disposes the machinery and the hover goes
...
pair one sector's board with another's slot; derived from one machinery at the build they cannot
...
machinery for machinery quietly makes a second copy that answers for a sector nothing draws.
...
the way the hover slot above is, by the sector's `SectorMapMachinery` through
