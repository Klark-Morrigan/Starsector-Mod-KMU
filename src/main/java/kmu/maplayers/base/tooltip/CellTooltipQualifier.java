 ...
 * optionally introduced by a word of the box's own and marked by a picture of what that finding
 * names.
 *
 * <p>One value rather than three parts layered onto a line, because the three are one statement. A
 * word, a mark and a name added separately could each be applied without the others, which leaves a
 * line free to end on a connective introducing nothing, or on a picture of something it never names -
 * fragments of an attribution no reader could complete.
 *
 * <p>Only the finding reads in the highlight the box reserves for what it has worked out; the word
 * introducing it and the mark beside it are quiet. So the ordinary qualifier - a single word about
 * the line, with nothing to introduce it - draws as the one gold run it has always drawn as.
 *
...
public record CellTooltipQualifier(
    ...
    // What a qualifier that is nothing but its finding carries where a word would introduce one.
    // Named rather than passed as a bare null, so the factory below says what the plain case has
    // none of instead of handing the constructor an unexplained absence.
     ...
     * and introduced by nothing.
     *
     * @param findingText what the box found about the thing on the line
     * @return the qualifier
     */
    public static CellTooltipQualifier stateFinding(String findingText) {
        return new CellTooltipQualifier(NO_LEADING_WORD, CellTooltipMark.NO_MARK, findingText);
    }

    /**
     * Builds a status introduced by a word of the box's own and marked by a picture of what it
     * names - what a line stating the thing it belongs to carries, such as the alliance a faction
     * was listed out of.
     *
    ...
    public static CellTooltipQualifier introduceFinding(
        ...
        return new CellTooltipQualifier(leadingWordText, mark, findingText);
    }
