...
public final class MapVisibilityPass {
    ...
    /**
     * Whether the system earns its place on the map in its own right rather than by the override
     * that shows hidden systems - {@link MapVisibility#hasOwnPlaceOnMap} over this pass's values.
     *
     * <p>Asked where two systems compete for something only one of them can have, a shared
     * hyperspace point among them. A system that fails this is on the map only while hidden systems
     * are being shown and leaves it the moment that goes off, so letting it take anything from a
     * system the map keeps either way would make what the second is drawn as depend on a toggle
     ...
     *
     * @param system the system to test; null has no place of its own, there being no system
     * @return true when the system would be on the map with nothing forced onto it
     */
    public boolean hasOwnPlaceOnMap(StarSystemAPI system) {

        return MapVisibility.hasOwnPlaceOnMap(system, visibleStars, isSystemInhabited(system));
    }
