 ...
 * has no date to give - what is being looked at is current by construction - and one nobody has
 * ever established has none either, being absent rather than old. So a row is dated by what it is
 ...
 * dated by the recalled one, rather than being left undated by the current one. Neither needs a
 * rule of its own, and neither can be broken by a later hand without breaking the one rule above.
 *
 * <p>Stated once for every family rather than per row, because two rows disagreeing about which of
 * their facts dates them would be read as two different claims about the world instead of as one
 * surface choosing differently twice.
 */
public final class ObservationNotes {
     ...
     *              to be settled by; a row carrying none yields no remark
