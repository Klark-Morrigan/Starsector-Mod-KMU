...
/**
 * Which of one location's colonies the player has yet to find, asked by the colony's own id.
 *
 ...
 *
 * <p>The unfound ones rather than the found ones, so an id the fold never met reads as found. The
 * answer is spent on calling a colony out as undiscovered, and a finding stated about a colony
 * nobody folded would be a claim the box has nothing behind.
 *
 ...
 * a listed row unanswered for reasons the caller had nothing to do with. Under the dev reveal that
 * is exactly the interesting case - a colony nobody has found is on the list and says so.
 *
 * @param undiscoveredColonyIds the market ids of the colonies whose entity the player has not found
 */
public record ColonyDiscoveryLookup(Set<String> undiscoveredColonyIds) {
    ...
    /**
     * Folds one location's colonies into the ones the player has yet to find.
     *
     ...
     * by a revelation gate has been found perfectly well, and one shown by a reveal has not.
     *
     * @param colonies the location's colonies, as one walk of it reported; null yields {@link #NONE}
     * @return the unfound ones among them, by colony id
     */
