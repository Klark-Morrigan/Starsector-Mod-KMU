 ...
 *
 * <p>The same loop the layers' watcher runs, on the same cadence, both taken from
 * {@link BaseStalenessSectorWatcher}, and a class of its own for the one reason that matters here:
 * the engine registers and clears transient scripts by exact class, so a poll installed and taken
 * back on its own timetable needs an identity of its own. {@link MapLayerSectorWatcher} states the
 * other half of that.
 *
 ...
 */
public class MapSubstrateSectorWatcher extends BaseStalenessSectorWatcher {
