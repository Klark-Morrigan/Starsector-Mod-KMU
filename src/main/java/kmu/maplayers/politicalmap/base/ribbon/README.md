...
that is [`base.render.ribbon`](../render/ribbon/README.md).
...
and judge their contests against one alliance set.

Lengths are counts of widths,
never world sizes:
how large a width is in the world is the render side's question,
which is what lets this whole package be exercised on literals.
...
That projection is the pass's `ColonyKnowledge` plus the one kind test habitation turns on:
the fog (`MarketVisibility.isCountedAsColony`),
the gates holding back the shapes a bare fog would leak,
and then the derelicts nobody has ever been aboard.
...
The rule is threaded rather than re-read because it is more than one bit now.
...
a colony the economy never listed reached neither band while both hover boxes named it,
a concealed colony fed a weight on one layer and a sibling term on the other,
and the dev reveal reached the held count
while the claim count went on hiding what the fill was drawing.
...
so every case above turns on hand-built values:
...
- `BlocPaletteReader` -
  ...
  It is [`base.render.style`](../render/style/README.md)'s rather than this package's:
  what colour a bloc is turned out not to be a question about bands,
  and the picker preview asks it too.
...
- **With a painter**,
  ...
  A system claimed by decree whose decreed bloc holds nothing there still bands,
  against the one rival present,
  because the fill names the decreed bloc and the band names somebody else.
- **With none** -
  an unclaimed cell,
  which no bloc's fill covers -
...
merging them is the alliances layer's job -
...
and every cell reads as it always has.

## What is not here

**The painter and the ranking.** Which bloc a cell was painted for,
and the order the blocs come out in,
are the painting mechanic's:
[`dominance.ribbon`](../../dominance/ribbon/README.md) ranks a held cell by the footprints its fill was decided from,
and [`claims.ribbon`](../../claims/ribbon/README.md) ranks a claims-layer cell by the contest over its system -
including the cells no claim covers,
which reach the rule above with no painter at all.
