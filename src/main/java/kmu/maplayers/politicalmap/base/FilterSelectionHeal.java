...
public final class FilterSelectionHeal {
     ...
     * nothing extra to invalidate. Reads the live sector and the player's current dominance and
    ...
    public static void healStaleSelectionAgainstActiveView() {
        for (var screenPicks : MapLayerScreens.getAllScreenPicks()) {
            ...
            var view = PoliticalMapViewRegistry.getSelectedView(screenPicks.memoryScope());

            // No view selected on this panel means no grouping to judge its slot under, so its stored
            // spotlight is left for a later view to rule on - the other panel is still healed.
            if (view == null) {
                continue;
            }
