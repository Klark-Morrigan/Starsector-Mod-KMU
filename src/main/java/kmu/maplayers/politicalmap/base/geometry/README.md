...
The shapes the political map is drawn out of. Every system the map draws gets one
...
Nothing here knows who owns anything. Ownership arrives as an opaque *grouping
key* per system, so the same geometry serves a map grouped by faction, by
alliance, or by claim.

Part of [the political map](../../README.md); see the
...
space closer to it than to any other. `PoliticalMapGeometryCache` builds and holds
