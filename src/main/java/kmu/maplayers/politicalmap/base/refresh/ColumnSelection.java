...
/**
 * How many columns the political-map filter picker lays its bloc list across, persisted per save. Like
 ...
 *
 * <p>Sidebar-only like the sort and filter selections: the count is driven solely by the picker's
 * columns selector, never a settings-screen control, so it persists in sector memory (each save keeps
 * its own choice and it survives reload) rather than as a LunaLib field - a LunaLib field would render
 * on a settings tab and an unregistered key would not round-trip.
 *
 * <p>Like the sort selection this fires no refresh: the column count only re-wraps the sidebar picker
 * list, which the tab rebuilds from live state every frame, so the next frame re-reads the stored key
 * and re-lays the rows on its own. Nothing on the map changes, so there is no overlay repaint to
 * request.
 */
public final class ColumnSelection {
    // Save-serialised key of the chosen column count; frozen once shipped, since renaming it silently
    // resets every existing save's column choice back to the default. Absent until the player first
    // picks a count, which the read reports as null for the column choice to default.
