...
/**
 * The two sides of one system's contest as an alliance set places them: everyone present who stands
 * with the bloc holding it, and everyone standing against it.
 *
 * <p>The same question every surface reporting a contest has to answer - a claim box routing
 * factions under its headings, a standings box routing ranked groups under its own - asked once
 * here so two surfaces over one system cannot put a bloc on different sides of it. What is being
 * placed and how the holder was found stay each surface's own: the contestants arrive as a list and
 * name their bloc through a reader, the holder having already been dropped from it.
 *
 * <p>The holder travels beside the alliance set rather than arriving per call, because both sides
 * of one contest are read against the one holder and passed apart they could be read against two.
 *
 * @param holderBlocId the bloc holding the system - the claimant, or the group the map painted the
 *                     cell for; no bloc at all where nobody holds it, which leaves every contestant
 *                     on the rival side, there being nobody for them to be standing with
 * @param affiliation  the alliance set the sides are placed under, which is
 *                     {@link BlocAffiliation#NONE} wherever nothing groups factions - every
 *                     contestant a rival, exactly as a contest read without alliances at all
 */
public record ContestSides(
     ...
     * that differs between them. The alliance set decides where a contestant lands:
     ...
     * {@link ContestSide#RIVAL} - a contest read exactly as one without alliances at all.
     *
