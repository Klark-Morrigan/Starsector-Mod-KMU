...
/**
 * The political map's terrain entity for Starscape mode: it resolves its spec and its plugin from
 * the political-map starscape terrain type it is constructed with, while reporting the one terrain
 * type the sector map draws over the starfield.
 *
 ...
 * entity is thus a political-map starscape terrain to the game's own resolution, and a slipstream
 * only to the map's draw filter.
 *
 ...
 * {@link StarscapeMapTerrainPlugin} instead, where it can be reached.
 */
public class StarscapeMapTerrain extends CampaignTerrain {
    ...
    /**
     * @param type the registered terrain type to resolve the spec and plugin from - the political
     *             map's starscape type, not the type this entity goes on to report
     */
    public StarscapeMapTerrain(String type) {
        // No plugin params: the political map's plugin is a map drawer that reads system positions
        // itself, so there is nothing to hand it at construction.
        super(type, null);
    }
