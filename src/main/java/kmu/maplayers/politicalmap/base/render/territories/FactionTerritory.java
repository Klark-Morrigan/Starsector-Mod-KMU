...
/**
 * One owned faction's fill and national border. The border is the GLU-resolved cluster of the
 * border rings; a territory that fills solid throughout has {@code fillTriangles} as that same
 * cluster triangulated, so the two match exactly, while a split territory's fills are each clipped
 * to that cluster (see below), so their outer edge lands on it too. {@code fillTriangles} is the
 ...
 * {@code GL_LINES} run of diagonal lines pre-clipped to the contested systems' cluster and baked
 ...
 * paints only its solid triangles. Each drawn state's cluster is clipped to the national border, so
 ...
 * ({@link kmu.maplayers.base.theme.GlobalStyle}) and the renderer sets it once,
 * not per territory.
 *
