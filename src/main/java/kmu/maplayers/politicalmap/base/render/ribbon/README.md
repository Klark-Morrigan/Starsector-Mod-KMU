...
alliance read is also the bands' only tie to whatever supplies alliances, answered with the identity
grouping wherever that mod is absent, so no rule below it carries a branch.

That map is `RibbonBakeSurface`: the inhabitation gate, the sites, the names' boxes and the rings,
read together off cells that have already been shaped and named, so a bake cannot be handed a gate
from one reading and sites from another. It also settles which cells get a band at all, in
