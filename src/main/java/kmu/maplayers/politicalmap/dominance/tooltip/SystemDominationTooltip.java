 ...
 * each colony's weight was summed from. The {@link MapHoverTooltip} the faction and alliance views
 * both inject, each holding one bound to its own grouping - it adapts flat vs nested off that grouping,
 * so those two views share one box that varies its content rather than each declaring its own.
 *
 ...
 *
 * <p>Stateless past the seams it is built around - the painting view's grouping, the live economy,
 * the alliance set and the settings are read afresh each paint.
 */
public final class SystemDominationTooltip extends SystemStandingsTooltip {
    ...
    public SystemDominationTooltip(
        ...
        super(paintedGroupingSource, claimBreakdownReader, holderGroupingSource, contestWordingSource);
    }
