 ...
 *
 * <p>Stateless past the seams it is built around - the live economy, the alliance set and the
 * settings are read afresh each paint. The grouping the ranking runs under is one of those seams
 * rather than a read of which view is up: the view that injects the box hands over its own, so a box
 * is always ranked under the view it was injected by, and no second layer's pick can reach it.
 */
public abstract class SystemStandingsTooltip extends PoliticalMapCellTooltip {
    ...
    @Override
    public final ComposedCellBody composeBody(
            SectorAPI sector,
    ...
    @Override
    public final HoverTooltipDetailLevel resolveDeepestHeldLevelFor(
            SectorAPI sector,
            StarSystemAPI system) {
